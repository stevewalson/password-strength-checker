const pswd_input = document.querySelector('.password'),
    ...
    copied_text = document.querySelector('.copied_text'),
...
generate.addEventListener('click', () => {
    var characters = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456788@*/\#&-+{%.(},?!",
        empty = "",
        random_length = Math.floor(Math.random() * (25 - 15 + 1) + 15);
    for (s = 0; s < random_length; s++) {
        pswd_input.value = empty += characters.charAt(Math.floor(Math.random() * characters.length));
    }
    pswd_length.innerText = "✅ Use at least 15 characters long password.";
    uppercase.innerText = "✅ Use at least one uppercase letter.";
    special_char.innerText = "✅ Use special Characters (e.g. @%...).";
